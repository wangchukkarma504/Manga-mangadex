...
@Component({
  ...
  template: `
    <div class="h-[100dvh] flex flex-col w-full max-w-md mx-auto bg-white dark:bg-dark-bg relative shadow-2xl overflow-hidden">
      ...
      <!-- Bottom Navigation (Hidden in Reader Mode) -->
      @if (showNav()) {
        <nav class="h-16 bg-white/95 dark:bg-dark-surface/95 backdrop-blur border-t border-gray-200 dark:border-gray-800 flex justify-around items-center z-50 shrink-0">
          <!-- Home -->
          <a routerLink="/" routerLinkActive="text-brand-500 dark:text-brand-400" [routerLinkActiveOptions]="{exact: true}" class="flex flex-col items-center justify-center w-full h-full text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors">
          ...
          <!-- Resume Reading (Smart Action) -->
          @if (lastRead()) {
            <a [routerLink]="['/read', lastRead()?.mangaId, lastRead()?.chapter]" class="flex flex-col items-center justify-center w-full h-full -mt-6">
          ...
          <!-- Favorites -->
          <a routerLink="/favorites" routerLinkActive="text-red-500" class="flex flex-col items-center justify-center w-full h-full text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 mb-1">
...
export class AppComponent {
  ...
  constructor() {
    this.router.events.pipe(
      ...
      // Hide nav bar if we are in reading mode
